...
# gene_pcs as well
#
# linear_f_test_worker for interaction test
...
linear_f_test_worker <- function(worker_id, ...) {
  # Read analysis configuration object.
  conf <- fromJSON(file = "analysis_configuration.json")

  # Read covariables.
  covars <- get_xs(conf) # age, genetic sex and PCs of genetic ancestry
  ...
  cat(paste0("variable_id,analysis_type,n_samples,rss_base,n_params_base,",
             ...
             "rss_augmented_inter,n_params_aug_inter,sum_of_sq_int,",
             "F_stat_inter,p_inter\n"),
  ...
  aug_int_cols <- conf$linear_conf$augmented_interaction_variables
  ...
  do.work <- function(metadata, data) {
    ...
    # The columns to extract for the covariables are juste the base_aug_cols,
    # since the interaction is between 'males' and the augmented columns
    m <- cbind(data[data_idx, outcome], covars[covars_idx, base_aug_cols])
    m <- m[complete.cases(m), ]

    # Find columns on rhs that have no variance after join.
    ...
    # Using formula API so column names are nicer in the output.
    ...
    int_formula <- as.formula(paste0(
      ylab, " ~ ", paste0(base_aug_int_cols, collapse = " + ")
    ))
    fit_int <- lm(int_formula, data = m)

    f <- anova(fit_base, fit_aug, fit_int)

    # Save the parameters as json.
    infer_df <- as.data.frame(summary(fit_int)$coefficients)
    ...
    line <- paste(
      ...
      f[2, 6],  # p
      f[3, 2],  # rss aug inter
      length(base_aug_int_cols) + 1, # n parameters base (+1 for intercept)
      f[3, 4],  # ssq inter
      f[3, 5],  # F inter
      f[3, 6],  # p inter
      sep = ","
